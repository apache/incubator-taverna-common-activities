...
public class JAXWSInvoker extends AbstractSOAPInvoker {
    ...
    public JAXWSInvoker(WSDLParser parser, String portName, String operationName) throws UnknownOperationException {
        super(parser, portName, operationName);
        ...
        for (QName service_name : parser.getServices()) {
            ...
            if (port_qname != null) {
                ...
                List<Handler> chain = new ArrayList();//Arrays.asList(new InvokerHandler());
...
//        rc.put(BindingProviderProperties.CONNECT_TIMEOUT, timeout);
    ...
    public Map<String, Object> invoke(Map<String, Object> inputs) throws Exception {
        ...
        createSoapBody(message, inputs);
        
        SOAPMessage response = call(message);
        
        return parseSoapBody(response);
    }
    
    /**
     * Prepares the soap message body filling it with provided input data
     * 
     * @param message the message for which the body is being constructed.
     * @param inputs message parameters map with xpath keys.
     * 
     * @throws SOAPException
     * @throws ParserConfigurationException
     * @throws XMLStreamException 
     */
    private void createSoapBody(SOAPMessage message, Map<String, Object> inputs) 
            throws XMLStreamException, TransformerConfigurationException, TransformerException, SOAPException {
        
        StringWriter w = new StringWriter();
        XMLOutputFactory streamFactory = XMLOutputFactory.newInstance();
        streamFactory.setProperty(XMLOutputFactory.IS_REPAIRING_NAMESPACES, Boolean.TRUE);
        XMLStreamWriter writer = streamFactory.createXMLStreamWriter(w);

        writeInputs(writer, inputs); // generate SOAP Body document

        TransformerFactory transformerFactory = TransformerFactory.newInstance();
        Transformer transformer = transformerFactory.newTransformer();
        transformer.transform(new StreamSource(new StringReader(w.toString())), new SAAJResult(message.getSOAPBody()));       
    }
    
    private Map<String, Object> parseSoapBody(SOAPMessage message) throws SOAPException, XMLStreamException, TransformerConfigurationException, TransformerException {
        ...
        XMLStreamReader reader = streamFactory.createXMLStreamReader(new StringReader(w.toString()));
        
        Map<String, Object> outputs = new HashMap();
        readOutputs(reader, outputs);
        return outputs;
    }
    
    private SOAPMessage call(SOAPMessage message) throws Exception {
